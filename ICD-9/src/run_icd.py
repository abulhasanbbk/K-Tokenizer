...
def parse_args():
    ...
    parser.add_argument("--result_save", type=str, default=None, help="Where to save the test result")
    #parser.add_argument("--percent_data", type=float, default=1, help="percent of data for training")
...
def main():
    ...
    num_part=1 # 5  parts for 20% of data, 3 parts for 30% of data, 2 parts for 50% of data 
    num_rows = len(df)
    ...
    for i, ds in  enumerate(datasets):
        ...
        if args.num_train_epochs == 0 and accelerator.is_local_main_process:
            ...
            seed_dir = os.path.join(args.result_save, f"seed_{args.seed}")
        ...
        if args.output_dir is not None and args.num_train_epochs > 0:
            ...
            unwrapped_model.save_pretrained(os.path.join(args.output_dir, f"seed_{args.seed}"), safe_serialization=False)
