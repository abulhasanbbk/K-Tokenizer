...
def parse_args():
    ...
    parser.add_argument("--corpus", type=str, required=True, help="You must provide K-Tokenizer corpus type")
    parser.add_argument("--tokenizer_base_loc", type=str, required=True, help="You must provide K-Tokenizer corpus location")
    parser.add_argument("--output_dir", type=str, default=None, help="Where to store the final model.")
    parser.add_argument("--seed", type=int, default=None, help="A seed for reproducible training.")
    parser.add_argument("--result_path", type=str, default=None, help="Where to save the test result")
    parser.add_argument("--full_result_path", type=str, default=None, help="Where to save the full classification report"  )
    parser.add_argument("--fertility", type=float, default=None, help="Set the fertility of the Ktokenizer"  )
    parser.add_argument("--percent_data", type=int, default=1, help="percent of data for training")
...
def main():
    ...
    if args.corpus=="umls":
        ...
        tokenizer=KTokenizerFromUMLS(locations, fertilities=0.0)
        ...
        locations.drug=os.path.join(args.tokenizer_base_loc, fileNames.base_loc_drug)
        locations.ktok=os.path.join(args.tokenizer_base_loc, fileNames.base_loc_ktok)
        tokenizer=KTokenizerFromMIMICIII(locations, fertilities=0.0)

    else:
        print("This program works only with umls or mimic corpus type for ICD-9 coding classification.")
        exit()
    ...
    def preprocess_function(examples):
        ...
        result = tokenizer(texts[0], batch=True)
        ...
        return result
    # print(subset_datasets["train"].column_names)
    ...
    #get the validation file (test file) adn construct validation dataset
    validation_file= args.validation_file
    val_df = pd.read_csv(validation_file)
    val_ds=Dataset.from_pandas(val_df)
    eval_dataset=val_ds.map(preprocess_function, batched=True)
    #get the parts for training 
    train_file=args.train_file
    #train_dataset = load_dataset(train_file, split=0.1)
    df = pd.read_csv(train_file)
    num_part=int(args.percent_data) # 5  parts for 20% of data, 3 parts for 30% of data, 2 parts for 50% of data 
    num_rows = len(df)
    partition_size = num_rows // num_part
    ...
    # Create a list of datasets from the partitions
    datasets = [Dataset.from_pandas(partition) for partition in partitions]
    eval_dataloader=None
    for i, ds in  enumerate(datasets):
        print(f"Dataset {i + 1} size: {len(ds)}")
        if args.num_train_epochs > 0: 
            train_dataset = ds.map(preprocess_function, batched=True)
        #if args.num_train_epochs > 0:
        ...
        if args.num_train_epochs > 0:
            ...
            model_save_dir = os.path.join(args.output_dir, f"seed_{args.seed}", f"part_{i}")
            model = model_class.from_pretrained(
                model_save_dir, 
                from_tf=bool(".ckpt" in args.model_name_or_path),
                config=config,
        ...
        if args.num_train_epochs == 0 and accelerator.is_local_main_process:
            ...
            all_labels = np.stack(all_labels)
            metrics = all_metrics(yhat=all_preds, y=all_labels, yhat_raw=all_preds_raw)
            metrics_dict = classification_report(all_labels, all_preds, target_names=label_list, output_dict=True)

            logger.info(f"evaluation finished")
            logger.info(f"metrics: {metrics}")

            for t in [0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]:
                all_preds_thresh = (all_preds_raw > t).astype(int)
                metrics_thresh = all_metrics(yhat=all_preds_thresh, y=all_labels, yhat_raw=all_preds_raw, k=[5, 8, 15])
                logger.info(f"metrics for threshold {t}: {metrics_thresh}")
                save_metrix[str(t)] = metrics_thresh

            save_metrix['final'] = metrics

            # === Save everything per seed with document IDs ===
            seed_dir = os.path.join(args.result_path, f"seed_{args.seed}", f"part_{i}")
            os.makedirs(seed_dir, exist_ok=True)

            # Get document IDs from validation set
            document_ids = val_df['HADM_ID'].tolist()
            assert len(document_ids) == len(all_preds), "Mismatch between number of documents and predictions!"

            # Save per-document CSV
            csv_path = os.path.join(seed_dir, "predictions_by_doc.csv")
            with open(csv_path, mode='w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(["document_id", "true_labels", "predicted_labels", "predicted_probs"])
                for i in range(len(document_ids)):
                    writer.writerow([
                        document_ids[i],
                        ";".join(map(str, np.where(all_labels[i] == 1)[0])),
                        ";".join(map(str, np.where(all_preds[i] == 1)[0])),
                        ";".join(f"{x:.6f}" for x in all_preds_raw[i])
                    ])

            # Save arrays
            np.save(os.path.join(seed_dir, "yhat_raw.npy"), all_preds_raw)
            np.save(os.path.join(seed_dir, "yhat.npy"), all_preds)
            np.save(os.path.join(seed_dir, "y.npy"), all_labels)

            # Save metrics
            with open(os.path.join(seed_dir, "classification_metrics.json"), 'w') as json_file:
                json.dump(metrics_dict, json_file, indent=4)

            with open(os.path.join(seed_dir, "metrics_summary.json"), 'w') as file:
                json.dump(save_metrix, file, indent=4)
