...
TRAIN_FILE_NAME="bio_train.txt"
CXR_TEST_FILE_NAME="bio_test_MIMIC_CXR_labeller1.txt"
CheXpert_TEST_FILE_NAME="bio_test_CheXpert_labeller1.txt"
...
def load_radgraph_data(location, word_col=1, tag_col=2):
    """
    Returns
        sents : list[list[str]]
        tags  : list[list[str]]
...
def run_ner(tokenizer, bert_model_loc, result_loc, model_loc,train_data_loc, test_data_loc, partition, corpus, seed):
    #out_dir="bert_models"
    ...
    print(test_data_loc)
    print("**********************************************************")
    train_sents, train_tags,train_ids=load_radgraph_data(train_data_loc)
    ...
    test_sents, test_tags, test_ids=load_radgraph_data(test_data_loc)
    ...
    for idx, part in enumerate(partitions_sents):
        ...
        training_args = TrainingArguments(
            ...
            num_train_epochs=20,                 # <-- only 3 epochs
            weight_decay=0.01,
            ...
            push_to_hub=False,
            save_total_limit=3,
            save_safetensors=False                  # keep only 3 checkpoints
        )
        ...
        trainer.train()
        trainer.evaluate()
        test_out = trainer.predict(test_dataset_tokenized)
        raw_logits = test_out.predictions
        pred_label_ids = np.argmax(raw_logits, axis=2)
        true_label_ids = test_out.label_ids  # shape (num_examples, max_seq_len)

        # Optionally, you might want to convert these back into BIO tags (strings) to save:
        all_pred_tags = [
            [label_list[p] for p, l in zip(pred_seq, true_seq) if l != -100]
            for pred_seq, true_seq in zip(pred_label_ids, true_label_ids)
        ]
        all_true_tags = [
            [label_list[t] for t in true_seq if t != -100]
            for true_seq in true_label_ids
        ]
        test_preds_path = os.path.join(result_part_dir_loc, f"test_preds_seed-{seed}.pkl")
        ...
        #        "labels":  true_labels,
        #        "sent_ids": sid_list     
        print(f"[Seed={seed}] Saved raw test predictions → {test_preds_path}")

   
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--corpus", type=str, required=True, help="Provide corpus type:base, umls or mimic ")
    parser.add_argument("--seed", type=int, required=True,  help="Provide 48, 1024, or 2048")
    parser.add_argument("--data_type", type=str, required=True, help="Provide type cxr or chexpert")
    args = parser.parse_args()
    set_seed(args.seed)
    fertilities=[0, 0.035,0.065,1.0]
    finetuned_save_loc=args.corpus+"_"+args.data_type+"_fine_tuned_model_"+str(args.seed)
    if args.corpus=="umls":
    ...
    train_data_loc=os.path.join(RADGRAPH_DATA_LOC, TRAIN_FILE_NAME)
    if args.data_type=="cxr":
        test_data_loc=os.path.join(RADGRAPH_DATA_LOC, CXR_TEST_FILE_NAME)
    elif args.data_type=="chexpert":
         test_data_loc=os.path.join(RADGRAPH_DATA_LOC,CheXpert_TEST_FILE_NAME)
    else:
        print("Please provide valid data type cxr or chexpert")
        exit()
    
    result_dir="results_"+args.data_type+"_"+args.corpus+"_"+str(args.seed)
